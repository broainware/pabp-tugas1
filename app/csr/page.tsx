...
export default function CSRPage() {
  ...
  // Ambil fungsi addToCart dan cartCount dari Context
  ...
  // Update fungsi handleOrder untuk mendukung Context API
  const handleOrder = (product: Product) => {
    if (product.stock > 0) {
      // Masukkan ke Keranjang Global 
      addToCart({ 
        id: product.id, 
        title: product.title, 
        price: product.price 
    ...
    <main className="min-h-screen font-sans selection:bg-[#d4c3a3] selection:text-[#3a0a0a] relative overflow-x-hidden bg-[#2a0505]">
      ...
      <div className="relative z-10">
        ...
        <div className="fixed bottom-10 right-10 z-[100] group">
          <div className="absolute -top-2 -right-2 bg-white text-[#1a0202] w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-black z-10 shadow-xl group-hover:scale-110 transition-transform">
            {cartCount}
          </div>
          <button className="bg-[#d4c3a3] text-[#1a0202] p-5 rounded-full shadow-[0_20px_50px_rgba(0,0,0,0.5)] border border-white/20 hover:bg-white transition-all">
          ...
          {/* Mini Tooltip */}
          <div className="absolute right-full mr-4 top-1/2 -translate-y-1/2 bg-black/80 backdrop-blur-md border border-white/10 px-4 py-2 rounded-sm opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none whitespace-nowrap">
             <span className="text-[9px] text-[#d4c3a3] font-black uppercase tracking-[0.2em]">Items in Bag</span>
          </div>
        </div>
